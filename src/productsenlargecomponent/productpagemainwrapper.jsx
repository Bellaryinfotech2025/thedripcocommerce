...
const Productpagemainwrapper = () => {
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(diff) > 50) {
      if (diff > 0) setCurrentSlide((prev) => (prev + 1) % allImages.length);
      else setCurrentSlide((prev) => (prev - 1 + allImages.length) % allImages.length);
    }
  ...
  return (
    <>
      ...
      <div className="tuple_nation_product_wrapper">
        ...
        <div className="tuple_nation_mobile_container">
          ...
          <div className="tuple_nation_mobile_purchase_panel">
            ...
            {stock < 10 && stock > 0 && (
              <p style={{ color: "#e65100", fontSize: "13px", marginTop: "12px", fontWeight: "500" }}>
        ...
        <section className="tuple_nation_section_three_container tuple_nation_desktop_only">
          ...
          <div className="tuple_nation_purchase_panel_lynx">
            ...
            <button
              ...
              style={{ opacity: selectedSize && !isLoading ? 1 : 0.5, cursor: selectedSize && !isLoading ? "pointer" : "not-allowed" }}
            >
              BUY NOW
            </button>
      ...
      <style jsx>{`
        .tuple_nation_mobile_container { display: none; }
        .tuple_nation_desktop_only { display: block; }

        .tuple_nation_mobile_slider { overflow: hidden; position: relative; width: 100%; background: #f5f1e8; }
        .tuple_nation_slider_inner { display: flex; transition: transform 0.4s ease-in-out; }
        .tuple_nation_slide { min-width: 100%; }
        .tuple_nation_mobile_image { width: 100%; height: auto; max-height: 80vh; object-fit: contain; }
        .tuple_nation_slider_dots { display: flex; justify-content: center; gap: 8px; padding: 12px 0; background: rgba(0,0,0,0.05); }
        .tuple_nation_dot { width: 8px; height: 8px; border-radius: 50%; background: #999; cursor: pointer; }
        .tuple_nation_dot_active { background: #000; }

        .tuple_nation_mobile_purchase_panel { padding: 1rem; background: #fff; }

        .tuple_nation_success_toast, .tuple_nation_error_toast {
          position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%);
          padding: 14px 32px; border-radius: 50px; font-weight: bold; z-index: 9999;
          box-shadow: 0 4px 20px rgba(0,0,0,0.3); font-size: 15px;
          animation: fadeInOut 3s forwards;
        }
        .tuple_nation_success_toast { background: #28a745; color: white; }
        .tuple_nation_error_toast { background: #ff4d4f; color: white; }
        ...
        @media (max-width: 768px) {
          .tuple_nation_mobile_container { display: block; }
          .tuple_nation_desktop_only { display: none !important; }
        }
      `}</style>
    </>
  );
